...
// Достижим ли порог 2PL при нынешней когорте - факт про УСТАНОВКУ, а не про элемент. Считается
...
$readycell = function (?object $rr) use ($cohort_reaches_2pl) {
    ...
    if ($rr->verdict === 'no_tags') {
        ...
        // не видно. Порог оценки дискриминации у сервиса намного выше нашего порога
        // достоверности, поэтому объясняем ноль.
        if ((int)$rr->ready_2pl_n === 0) {
            if (!$cohort_reaches_2pl) {
                // Порог недостижим при нынешней когорте - обратный отсчет обещал бы невозможное.
                // Условие НЕ привязано к to_2pl_n: на пилоте, где еще ничего не калибровано,
                // отсчет равен нулю, и методист оставался бы вовсе без объяснения (найдено ревью).
                $line .= ', 2PL недостижим при нынешнем числе учащихся';
            } else if ((int)$rr->to_2pl_n > 0) {
                // Именно «ближайшему заданию»: по поддереву берется максимум, и без этой
                // оговорки число читалось бы как готовность всего раздела.
                //
                // Единица - УЧАЩИЕСЯ, а не ответы: наблюдение в калибровке это пара
                // «ученик-задание», повторные попытки одного ребенка схлопываются.
                //
                // «Не хватает» вместо «нужно»: при N на единицу («191 учащийся») именительный
                // падеж давал безграмотную фразу. Родительный после «не хватает» верен при любом N.
                $n = (int)$rr->to_2pl_n;
                $line .= ', ближайшему заданию не хватает еще ' . $n . ' '
                    . local_unics_plural($n, 'учащегося', 'учащихся', 'учащихся');
            } else if ((int)$rr->flat_2pl_n > 0) {
                // Ответов набралось, дискриминация оценена и вышла около единицы: ждать нечего,
