...
$readycell = function (?object $rr) {
    ...
    if ($rr->verdict === 'no_tags') {
        ...
        if ((int)$rr->ready_2pl_n === 0) {
            if ((int)$rr->to_2pl_n > 0 && empty($rr->to_2pl_reachable)) {
                // Порог недостижим при нынешнем числе детей: обратный отсчет обещал бы
                // невозможное. Честнее сказать прямо, чем показывать «еще 184» там, где
                // учащихся шестнадцать (найдено ревью).
                ...
                // «ученик-задание», повторные попытки одного ребенка схлопываются. Прежний текст
                // говорил «ответов» и завышал достижимость.
                $n = (int)$rr->to_2pl_n;
                $line .= ', ближайшему заданию нужно еще ' . $n . ' '
                    . local_unics_plural($n, 'учащийся', 'учащихся', 'учащихся');
