...
if (empty($records)) {
    ...
    foreach ($records as $r) {
        ...
        if ($r->status == 4 && $r->error_message) {
            // Срезаем префикс «error/»: он остается у СТАРЫХ записей очереди, снятых до
            // 2026-08-25. Новые отказы сервисов ИИ идут через ai_error::exception() с
            // generalexceptionmessage и префикса не несут ([[ai-error-messages-design]]),
            // но в базе лежит история, и педагогу этот префикс говорит только о том, что
            // что-то сломано в коде.
