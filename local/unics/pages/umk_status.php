...
if (empty($records)) {
    ...
    foreach ($records as $r) {
        ...
        if ($r->status == 4 && $r->error_message) {
            // Срезаем префикс «error/»: почти все throw в плагине передают русское предложение
            // первым аргументом moodle_exception, который трактуется как идентификатор языковой
            // строки. Педагогу этот префикс говорит только о том, что что-то сломано в коде.
            // Настоящее лечение - языковые строки во всех throw, это отдельная задача.
            $msg = preg_replace('~^error/~', '', (string)$r->error_message);
            $status .= '<br><small class="text-danger">' . s($msg) . '</small>';
        }
