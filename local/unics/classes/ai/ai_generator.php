...
class ai_generator {
    ...
    /**
     * Сколько спорных вопросов в комплекте должны разом лечь на «ключ правее выбора судьи»,
     * чтобы мы поверили в сбитый счет вариантов и подвинули ключи, а не выбросили вопросы.
     *
     * Одному совпадению верить нельзя: судья ошибается, и случайная ошибка попадает на соседний
     * вариант примерно в трети случаев. А два и три разом - это уже привычка модели в этом
     * комплекте. Проба 2026-08-27 на сорока вопросах: расхождений с ключом восемь, и семь из них
     * легли ровно на correct-1, причем ВСЕ семь - в комплектах, где таких было двое или трое
     * ([[one-based-key-design]], раздел 9).
     */
    public const JUDGE_ONEBASED_MIN = 2;
...
correct - НОМЕР правильного ответа, считая С НУЛЯ: первый вариант - 0, второй - 1, третий - 2, четвертый - 3. Числа 4 и больше недопустимы.";
    ...
    private function judge_survivors(array $survived): array {
        ...
        // Спорные вопросы, где выбор судьи лег ровно на вариант ПЕРЕД ключом. Это тот же счет
        // с единицы, что и в разборе, только внутри диапазона: там его видно по значению, а тут
        // не видно ничем, кроме независимого мнения судьи. Существенно, что судья слеп к нашему
        // порядку - варианты ему перемешиваются, и отвечает он текстом, - поэтому совпадение с
        // correct-1 не может быть артефактом его нумерации.
        $shiftcandidates = [];
        foreach ($survived as $i => $q) {
            ...
            if (!empty($q['onebased'])) {
                // Этому вопросу ключ уже двигали при разборе. Второй сдвиг в ту же сторону -
                // не исправление, а разгон: если после первой правки судья все равно спорит,
                // вернее выбросить вопрос, чем угадывать дальше.
                continue;
            }
            $at = $at_list[$i] ?? null;
            $key = (int)($q['correct'] ?? 0);
            if ($at !== null && $key > 0 && $at === $key - 1) {
                $shiftcandidates[$i] = $at;
            }
        }
        // Порог: чиним только привычку комплекта, а не одиночное совпадение.
        $fixshift = count($shiftcandidates) >= self::JUDGE_ONEBASED_MIN;
        if ($shiftcandidates && !$fixshift) {
            $this->trace('  Спорный вопрос похож на сбитый счет вариантов, но одиночному '
                . 'совпадению не верим - вопрос отброшен');
        }
        ...
        foreach ($survived as $i => $q) {
            if (($verdicts[$i] ?? 'ok') === 'drop') {
                if ($fixshift && isset($shiftcandidates[$i])) {
                    // Отбраковка превращается в починку: вопрос годный, сбит только ключ.
                    $q['correct'] = $shiftcandidates[$i];
                    $byshift++;
                    unset($q['computed']);
                    $kept[] = $q;
                    continue;
        ...
        if ($byshift) {
            $this->trace('  Ключ переставлен по выбору судьи: ' . $byshift
                . ' (в комплекте сбит счет вариантов, вопросы сохранены вместо отбраковки)');
        }
