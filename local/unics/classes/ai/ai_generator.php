...
class ai_generator {
    ...
    public function build_criteria(array $profile): array {
        ...
        $ovz_instructions = [
            // Замер 2026-09-04: указания для видов 1, 2, 3 и 6 не меняли выход НИ ПО ОДНОМУ
            // своему обещанию (p от 0.14 до 1.00 против контроля). Причина у видов 1 и 2 -
            // ЗАПРЕТ НЕСУЩЕСТВУЮЩЕГО: зрительных отсылок в контроле 0.00 на текст, звуковых
            // тоже 0.00, то есть запрещалось то, чего модель и так не делает. Ровно та же
            // форма, что у запрета отрицаний в вопросах ([[ovz-adaptation-measured]]).
            // Запрет заменен положительным требованием: сказать, что ДЕЛАТЬ, а не чего не делать.
            //
            // ЧЕСТНО ПРО ПРЕДЕЛ: новая редакция вида 1 ПЕРЕМЕРЕНА и сдвига тоже не дала
            // (p = 0.17), а вид 2 после переписывания не мерился вовсе. Артефакта у обоих нет,
            // проверять нечем, и утверждать «текст адаптируется» для них нельзя. Настоящий барьер
            // слабовидящего снят не здесь, а в подписи иллюстрации ([[ovz-adaptation-measured]],
            // раздел 11): картинка теперь называет себя, а не повторяет заголовок.
            1 => 'Текст должен быть понятен без опоры на изображение: то, что обычно показывают '
               . 'рисунком, опиши словами прямо в тексте.',
            2 => 'Всё необходимое для понимания содержится в письменном тексте, без опоры на '
               . 'звучащее.',
            // НОДА: указание обещало «чёткие пошаговые инструкции», а плечо дало 0.00
            // нумерованных шагов при 0.80 в контроле - хуже, чем без указания. Обещание стало
            // проверяемым артефактом.
            // Требование БЕЗУСЛОВНОЕ, как у ЗПР и РАС. Первая редакция начиналась с «Любой
            // порядок действий», то есть на описательной теме («Виды горных пород») модель
            // законно писала ноль шагов - а проверка все равно требовала три, жгла переспрос на
            // каждом таком комплекте и писала в след ложный провал (найдено ревью).
            //
            // Разбить на шаги можно ЛЮБОЕ объяснение, не только процесс: при НОДА барьер - объем
            // удерживаемого и требование скорости, и ступенчатая подача снимает его независимо от
            // темы.
            3 => 'Объяснение подавай ступенями: пронумерованными строками «' . self::STEP_MARKER
               . ' 1. ...», «' . self::STEP_MARKER . ' 2. ...» - не меньше '
               . self::STEP_MIN . ' шагов, даже если тема описательная. Не требуй от учащегося '
               . 'быстрых действий.',
            // Замер 2026-09-02 ([[ovz-adaptation-measured]]): прежняя редакция не меняла выход
            // ВОВСЕ. «Короткие абзацы» контроль выполнял и без указания (2.42 предложения), а
            ...
            //
            // Слово артефакта берется из MEMO_MARKER, а не пишется здесь буквой: иначе смена
            // константы обновила бы проверку и переспрос, но НЕ саму просьбу, и каждый текст ЗПР
            // стоил бы лишней генерации, не проходя проверку никогда (найдено ревью).
            ...
            // «Короткие предложения» контроль выполняет и без указания (8.25 слова), а «минимум
            // терминов без пояснений» было непроверяемым: доля поясненных терминов в плече 31.9%
            // против 30.0% в контроле.
            //
            // Редакция ниже конкретнее, но ЭФФЕКТА ТОЖЕ НЕ ДАЛА: 37.0% против 30.0%, p = 0.74.
            // Счетчика для нее нет, артефакта нет, порога нет - то есть это по-прежнему пожелание,
            // и первая версия этого комментария зря обещала «можно посчитать» (найдено ревью).
            // Оставлено как есть: вреда нет, а измеримой адаптации для вида 6 добиться не удалось.
            //
            // Требования «короткие предложения» и «поясняй термин в той же фразе» тянут в разные
            // стороны - пояснение удлиняет ровно то предложение, которое просят сократить. При
            // нулевом эффекте спор безобиден, но если у вида 6 когда-нибудь появится артефакт,
            // разводить оси придется как в [[criteria-conflicts-design]].
            6 => 'Доступный язык, короткие предложения. Каждый выделенный жирным термин поясняй '
               . 'СРАЗУ ЖЕ, в той же фразе, простыми словами.',
        ];
        ...
        return [
            ...
            // intval - как у category_ids строкой выше. Без него профиль, пришедший через JSON или
            // импорт (['4'] вместо [4]), давал бы указание для ЗПР в промте и ярлык ЗПР в
            // предпросмотре, но memo_required() с его строгим сравнением вернул бы false: все
            // признаки говорят «адаптация включена», а проверки нет (найдено ревью).
            'ovz_type_ids'        => array_values(array_map('intval', $valid_ovz_types)),
    ...
    /**
     * Артефакт для ребенка с нарушением двигательного аппарата ([[ovz-adaptation-measured]]).
     *
     * Барьер при НОДА - не сложность материала, а объем действия и требование скорости. Указание
     * это и обещало («четкие пошаговые инструкции»), но замер показал 0.00 нумерованных шагов при
     * 0.80 в контроле: с указанием выходило ХУЖЕ, чем без него. Обещание сделано проверяемым.
     */
    public const STEP_MARKER = 'Шаг';

    /** Сколько шагов делает порядок действий разобранным, а не просто названным. */
    public const STEP_MIN = 3;
     ...
     * 4-5 штук на текст. Сползание модели к одному итоговому «Запомни: подведем итог» - это откат
     * адаптации, и проверка на наличие его пропустила бы (найдено ревью). Порог держим низким:
     * ловим схлопывание к одному, а не считаем разделы - число разделов задает сама модель, и
     * жесткая привязка к нему гнала бы переспрос за разметку, а не за суть.
     */
    public const MEMO_MIN = 2;

    /**
     * ВСЕ правила артефакта в ОДНОМ месте: кому нужен, сколько штук, как выглядит хвост метки,
     * что просить при переспросе.
     *
     * Раньше эти четыре свойства жили в трех разных методах, и тест сторожил только одно из них
     * (формулировку переспроса). Артефакт с меткой вида «Итог - 1)» молча унаследовал бы общий
     * хвост «метка плюс двоеточие», считался бы вечно нулем, и КАЖДЫЙ комплект платил бы
     * переспросом без единого падающего теста (найдено ревью). Хвост - такое же свойство
     * артефакта, как порог, и лежать обязан рядом с ним.
     *
     * `tail` - хвост шаблона ПОСЛЕ метки; `%d` в `ask` подставляется порогом.
     */
    private const ARTIFACT_SPEC = [
        self::MEMO_MARKER => [
            'ovz'  => 4,
            'min'  => self::MEMO_MIN,
            'tail' => '\s*:',
            'ask'  => 'не меньше %d строк «%s: ...», в каждой - главное понятие своего раздела '
                    . 'короткой фразой',
        ],
        self::EXAMPLE_MARKER => [
            'ovz'  => 5,
            'min'  => self::EXAMPLE_MIN,
            'tail' => '\s*:',
            'ask'  => 'не меньше %d строк «%s: ...», в каждой - короткий пример из реальной жизни '
                    . 'к своему разделу',
        ],
        self::STEP_MARKER => [
            'ovz'  => 3,
            'min'  => self::STEP_MIN,
            // Шаг пишется «Шаг 1.», с номером и точкой. Пробелы НЕ переходят через перенос
            // строки: иначе проза «...первый шаг» и следующая строка «1. Нагреть воду»
            // склеивались в совпадение, и три таких стыка брали порог без единого блока.
            'tail' => '[ \t]*\d+[ \t]*[.):]',
            'ask'  => 'не меньше %d пронумерованных строк «%s 1. ...», «%s 2. ...» с порядком '
                    . 'действий',
        ],
    ];
     ...
     * каждого раздела снимает его. Прочим видам нарушения артефакт сам по себе не нужен.
     *
     * СОЧЕТАНИЕ ЗПР+РАС - решение сознательное, а не случайное «истина» (ревью справедливо
     * спросило). Артефакт остается: повтор после КАЖДОГО раздела - это и есть предсказуемая
     * структура, которой требует указание для РАС, а не нарушение ее. Нарушал бы повтор, который
     * появляется то тут, то там; ровный - наоборот, опора.
     */
    public function memo_required(array $criteria): bool {
        return $this->artifact_required(self::MEMO_MARKER, $criteria);
    }

    /**
     * Нужен ли артефакт по критериям. Вид нарушения берется ИЗ ТАБЛИЦЫ, а не пишется числом в
     * каждом предикате: два места с одним и тем же правилом расходятся на первой правке.
     */
    private function artifact_required(string $marker, array $criteria): bool {
        $ovz = self::ARTIFACT_SPEC[$marker]['ovz'] ?? null;
        return $ovz !== null
            && in_array($ovz, array_map('intval', $criteria['ovz_type_ids'] ?? []), true);
    }
    ...
    public function example_required(array $criteria): bool {
        return $this->artifact_required(self::EXAMPLE_MARKER, $criteria);
    }
    ...
    public function required_artifacts(array $criteria): array {
        $types = array_map('intval', $criteria['ovz_type_ids'] ?? []);
        $need  = [];
        foreach (self::ARTIFACT_SPEC as $marker => $spec) {
            if (in_array($spec['ovz'], $types, true)) {
                $need[$marker] = $spec['min'];
            }
        }
        return $need;
    }
     ...
     * и «**Запомни**: ...» - самая вероятная запись артефакта. Голый шаблон ее не видел, а значит
    ...
    public function artifact_count(string $text, string $marker): int {
        ...
        // Якорь по границе слова ОБЯЗАТЕЛЕН, и не для красоты: «пример» - подстрока слова
        // «например», а «например» - обычнейшая связка, которую вдобавок приглашает само общее
        // требование про примеры. Без якоря текст с тремя «Например: ...» и НУЛЕМ строк-примеров
        // проходил проверку, а след рапортовал об успехе - то есть проверка возвращала ровно то
        // «непроверяемое пожелание», ради устранения которого написана (найдено ревью).
        // На 20 текстах замера «Например:» не встретилось ни разу, так что числа замера верны -
        // дыра была скрытой, как и с разметкой.
        //
        // Хвост метки берется из ARTIFACT_SPEC: он свойство артефакта, а не частный случай
        // внутри метода, который притворяется общим.
        $tail = self::ARTIFACT_SPEC[$marker]['tail'] ?? '\s*:';
        return preg_match_all('/(?<![\p{L}])' . preg_quote($marker, '/') . $tail . '/iu', $flat);
    }
    ...
    /**
     * Убрать строки-ПОВТОРЫ из текста, который идет ИСТОЧНИКОМ другим генерациям.
     *
     * Режется только «Запомни», но не «Пример»: ревью справедливо заметило, что примеров тоже
     * около пяти на текст и они тоже едят окно. Разница в том, ЧТО это за строки. Повтор не несет
     * ничего нового - он пересказывает соседний абзац, и в источнике для теста он чистая потеря.
     * Пример - новое сведение, по нему можно задать вопрос, и вырезать его значило бы обеднить
     * тест ровно для того ребенка, которому конкретика нужнее всего. Решение сознательное, на него
     * есть отдельный тест.
     *
     * Тест, задание и видеосценарий получают учебный текст обрезанным по 1500-2000 знаков, а текст
     * для ЗПР и так укладывается в ~2100. Четыре-пять строк повтора - это больше десятой части
     * окна, и они вытесняли бы за срез настоящий материал: ребенок получил бы вопросы по началу
     * урока и ни одного по концу (найдено ревью). Тот же прием уже применен к разметке картинок -
     * она живет в отдельной переменной и в источник не попадает.
     *
     * Озвучку это НЕ касается: там повтор вслух работает ровно на то, ради чего артефакт и введен.
     */
    public function strip_memo(string $text): string {
        $lines = preg_split('/\R/u', $text);
        $kept  = array_filter($lines, fn(string $l): bool => $this->memo_count($l) === 0);
        return trim(implode("\n", $kept));
    }
     ...
     *
     * ИНВАРИАНТ: учебный текст генерируется ТОЛЬКО через этот метод. Прямой вызов generate_text()
     * для урока вернет непроверенный текст - ровно тот дефект, ради которого замер и делался.
     *
    ...
    public function generate_lesson_text(array $criteria, string $prompt): string {
        ...
        // Каждый артефакт называем ОТДЕЛЬНО и с двоеточием. Склейка через implode давала для
        // ребенка с ЗПР и РАС разом строку «строки «Запомни», «Пример: ...»»: в нужном виде стоял
        // только последний, и модель, послушавшись буквально, писала «Запомни» без двоеточия -
        // ровно то, чего проверка не засчитывает (найдено ревью).
        //
        // Требование к СОДЕРЖАНИЮ строки обязательно: проверка считает метки, поэтому без него
        // переспрос выполнялся бы наполнителем вида «Запомни: далее», и адаптация снова стала бы
        // декорацией. Число тоже называем - иначе модель ставит по одному на раздел, разделов
        // выходит два, и порог не берется при честном соблюдении.
        $wanted = [];
        foreach ($missing as $marker => $min) {
            // Требование к СОДЕРЖАНИЮ у каждого артефакта свое. Прежняя развилка знала два
            // случая и третьему выдала бы формулировку про примеры; match заставляет назвать
            // новый артефакт явно, а не получить чужой текст молча.
            // Формулировка берется из ARTIFACT_SPEC. Прежний match знал случаи поименно, и
            // артефакт без своей ветки получал чужой текст либо ронял генерацию урока целиком
            // (UnhandledMatchError летит ВНЕ try). Запасная строка не выдумывает содержания -
            // она просит только форму, а тест сторожит, что до нее не доходит.
            $spec = self::ARTIFACT_SPEC[$marker] ?? null;
            $wanted[] = $spec !== null
                ? sprintf($spec['ask'], $min, $marker, $marker)
                : sprintf('не меньше %d строк «%s: ...»', $min, $marker);
        }
        try {
            ...
            $this->trace('Переспрос за артефактами «' . implode('», «', array_keys($missing))
                . '» не удался: ' . $e->getMessage() . '. Оставлен первый текст.');
            return $text;
        }

        $left = $this->missing_artifacts($retry, $criteria);
        if (!$left) {
            return $retry;
        }
        // Берем ТОТ текст, где артефактов не хватает меньшему числу видов: ребенку с ЗПР и РАС
        // разом половина адаптации лучше, чем ее отсутствие. Признак - булев, а не сравнение
        // строк: два одинаковых ответа модели (низкая температура, заглушка) делали сравнение
        // истинным, и след врал, будто отдан второй текст (найдено ревью).
        $useretry = count($left) < count($missing);
        // Счета в следе обязательны: без них «2 при пороге 3» неотличимо от «0», то есть нельзя
        // понять, порог ли слишком высок или указание не выполняется вовсе.
        $counts = [];
        foreach (array_keys($this->required_artifacts($criteria)) as $marker) {
            $counts[] = $marker . ': ' . $this->artifact_count($text, $marker) . ' и '
                . $this->artifact_count($retry, $marker);
        }
        $this->trace('Артефакты не набраны и после переспроса (' . implode('; ', $counts)
            . '; пороги ' . implode(', ', $this->required_artifacts($criteria)) . '). '
            . 'Урок отдан ' . ($useretry ? 'со вторым' : 'с первым') . ' текстом.');
        return $useretry ? $retry : $text;
    }
    ...
    public function build_prompt(array $profile, string $topic, string $extra_context = ''): string {
        ...
        // Число примеров задается ОДНОЙ осью. Для РАС его диктует блок адаптации (пример в каждом
        // разделе), и общий потолок «2-3» с ним спорил: модель, послушавшая потолок, не проходила
        // проверку и стоила лишней генерации на каждом комплекте. Та же беда, что была с «углубляй»
        // рядом с «делай проще» ([[criteria-conflicts-design]], найдено ревью).
        $examples = $this->example_required($c)
            ? '- Примеры из реальной жизни или природы - в каждом разделе, как сказано выше'
...
Верни только текст задания. Без заголовков, без вводных слов - только само задание.";
    ...
    // ----------------------------------------------------------------
    // Генерация сценария видеопрезентации (просим VIDEO_SLIDES, возвращаем сколько пришло целыми)
    // Возвращает массив: [['title'=>..., 'content'=>..., 'key_points'=>[...]], ...]
    // ----------------------------------------------------------------
    /** Сколько слайдов просим у модели. Промт и проверка полноты обязаны знать одно число. */
    public const VIDEO_SLIDES = 5;

    /**
     * Потолок токенов сценария.
     *
     * Было 3000 на пять слайдов - 600 на слайд, ниже и пола теста (4096), и его бюджета на
     * вопрос. Живая проба дала обрыв на одной генерации из десяти, а по строению промта теряется
     * ВСЕГДА последний слайд, то есть «Итог» с главным выводом и вопросом для размышления: урок
     * ребенка обрывался без завершения. Разбор оборванного ответа - лечение симптома; причина в
     * потолке ([[video-script-json-2026-09-06]]).
     */
    private const VIDEO_TOKENS = 4096;
    ...
    public function generate_video_script(array $profile, string $topic,
        ...
        // них: модель пишет около двух предложений и с ОВЗ, и без него. Число приведено к
        // наблюдаемому и к более строгому указанию, чтобы промт хотя бы не спорил сам с собой.
        //
        // ДВА ЗАМЕРА РАСХОДЯТСЯ, и это про ИЗМЕРИТЕЛЬ, а не про модель: 2026-08-31 вышло 2.0 и
        // 1.9 предложения на слайд, 2026-09-06 - 1.56 и 1.52 на том же промте. Четверть разницы
        // при одинаковых условиях означает, что счетчики предложений были разные (второй
        // отбрасывал огрызки короче двух слов). Вывод у обоих ОДИН - разницы между плечами нет, -
        // и держится он на сравнении внутри одного прогона, а не на абсолютном числе. Абсолютному
        // числу тут верить нельзя (найдено ревью).
        //
        // ДОЛГ «объем слайда не адаптируется» ЗАКРЫТ ЗАМЕРОМ 2026-09-06, и закрыт отрицательно:
        // адаптировать тут нечего. Пять сценариев на плечо, по пять слайдов в каждом -
        // предложений на слайд 1.52 в контроле против 1.56 у ЗПР (p = 1.000), слов 11.1 против
        // 13.0 (p = 0.087, то есть у ЗПР скорее ДЛИННЕЕ). Формат слайда сам держит объем около
        // дюжины слов: короче - это уже заголовок, а не содержание. Артефакта здесь не будет,
        // и говорить «объем адаптируется» нельзя ([[ovz-adaptation-measured]]).
        $block     = $this->adaptation_block($c);
        $extra     = $this->extra_block($extra_context);

        // Число в промте и в проверке полноты - одно и то же: разъедутся, и след будет
        // сообщать о неполноте там, где модель выполнила просьбу дословно.
        $slidecount = self::VIDEO_SLIDES;
...
{$block}{$extra}
Верни РОВНО {$slidecount} слайдов в формате JSON без пояснений и без markdown-обёртки:
...
5. Итог - главный вывод и вопрос для размышления";
        ...
        $raw = $this->generate_text($prompt, self::VIDEO_TOKENS);

        // Разбор ОБЩИМ json_reply, а не своей жадной скобкой. GigaChat портит JSON тремя
        // способами, и все три этот класс уже умеет; тест давно ходит через него (строка с
        // 'questions'), а видеосценарий держал свою копию из жадной «{.*}» плюс одной правки
        // экранирования.
        //
        // Копия падала на ТРЕТЬЕМ способе - обрыве ответа на лимите токенов: жадный шаблон хватал
        // от первой скобки до ПОСЛЕДНЕЙ, а последняя лежала внутри оборванного слайда, и разбор
        // проваливался целиком. Ребенок терял весь видеосценарий, педагог видел «ИИ вернул
        // некорректный формат». Поймано живой пробой: 1 отказ на 10 генераций. На том же ответе
        // json_reply достает 4 слайда из 5, а конвейер работает по фактическому их числу.
        $data = json_reply::decode($raw, 'slides');

        if (!isset($data['slides']) || !is_array($data['slides'])) {
            // Начало ответа НЕ годится для диагноза: по первым тремстам знакам не отличить обрыв
            // от порчи разметки, а причина всегда в конце (json_reply::head_and_tail, куплено
            // живым заходом 2026-08-20). Тест ходит через нее давно, видеосценарий - нет.
            throw new \moodle_exception('generalexceptionmessage', 'error', '',
                'ИИ вернул некорректный формат видеосценария: ' . json_reply::head_and_tail($raw));
        }

        // Проверка полноты применяется ТОЛЬКО к оборванному ответу. Первая редакция применяла ее
        // всегда и уронила два чужих теста: слайд с законным содержимым без точки в конце
        // отбрасывался, и сценарий выходил пустым. Отбрасывать целое из-за пропущенной точки
        // хуже, чем пропустить обрывок: обрывки бывают лишь у оборванного ответа.
        $truncated = !self::brackets_balanced($raw);
        ...
        foreach ($data['slides'] as $s) {
            ...
            // Обрыв ответа портит ПОСЛЕДНИЙ слайд, и восстановление честно закрывает скобки: в
            // материал уходил обрывок вида «Текс» - ребенок видел его на слайде, а озвучка читала
            // вслух (найдено ревью, воспроизведено на срезах -40 и -50). До перехода на
            // json_reply терялся весь сценарий; менять потерю на мусор нельзя, поэтому неполный
            // слайд отбрасывается, а остальные остаются.
            if ($truncated && !self::slide_content_complete($content)) {
                $dropped++;
                continue;
            }
            $result[] = [
        ...
        // След обязателен: без него восстановленный сценарий неотличим от чистого прогона, и
        // частоту обрывов нельзя измерить второй раз. Рост с одного случая на десять до одного на
        // два выглядел бы в журнале ровно так же, то есть никак.
        if ($dropped > 0 || ($truncated && count($result) < self::VIDEO_SLIDES)) {
            $this->trace('Видеосценарий пришел неполным: слайдов ' . count($result) . ' из '
                . self::VIDEO_SLIDES . ', отброшено оборванных ' . $dropped
                . '. Обычная причина - упор в лимит токенов.');
        }
    ...
    /**
     * Сошелся ли баланс скобок - признак того, что ответ дошел целиком.
     *
     * Проверять «кончается ли ответ на скобку» мало: обрыв по границе объекта тоже кончается на
     * «}», хотя внешний массив и объект так и не закрылись. Баланс это различает.
     *
     * Строки пропускаем вместе с экранированием: скобка внутри текста слайда - обычное дело и
     * балансом не является.
     */
    private static function brackets_balanced(string $raw): bool {
        $depth = 0;
        $instr = false;
        $esc   = false;
    ...
    /**
     * Похоже ли содержимое слайда на ЦЕЛОЕ, а не на обрывок восстановленного ответа.
     *
     * Признак ОДИН: завершающий знак. Сперва рядом стоял и порог длины, но мутация показала, что
     * он не работает - оба живых обрывка («Текс», «Текст слайда 5») отсекаются знаком, а не
     * длиной. Непроверяемое условие вдобавок вредно: короткий, но ЦЕЛЫЙ слайд оно отбросило бы зря.
     *
     * Строгость намеренно умеренная: цель не оценить качество текста, а не пустить в материал
     * заведомо недописанное. Обрыв ровно по точке дает целое предложение - его и оставляем.
     */
    private static function slide_content_complete(string $content): bool {
