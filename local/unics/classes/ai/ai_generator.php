...
{\"questions\":[{\"text\":\"Текст вопроса?\",\"answers\":[\"Вариант А\",\"Вариант Б\",\"Вариант В\",\"Вариант Г\"],\"answer\":\"Вариант Б\",\"solution\":\"вычисление или пусто\"}]}
answer - ДОСЛОВНЫЙ ТЕКСТ правильного варианта, скопированный из списка answers знак в знак. НЕ номер и НЕ пересказ.";
    ...
    private function questions_from_reply(string $raw, int $num, array &$surplus = []): array {
        ...
        foreach ((array)($data['questions'] ?? []) as $q) {
            ...
            $bytext = false;
            $correct = null;
            $wanted = trim((string)($q['answer'] ?? ''));
            if ($wanted !== '') {
                $norm = array_map([question_sanity::class, 'option_text'], $answers);
                ...
                // одинаковых текстов имела в виду модель, неизвестно. Такие вопросы все равно
                // выбьет question_sanity, но гадать до нее незачем.
            ...
            $onebased = false;
            if (!$bytext && $correct === count($answers) && $correct > 0) {
                $correct--;
                $onebased = true;
            }
        ...
        foreach ($kept as $q) {
            ...
            unset($q['wasfixed'], $q['notes'], $q['onebased']);
