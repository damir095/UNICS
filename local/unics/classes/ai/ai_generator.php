...
class ai_generator {
    ...
    public function artifact_count(string $text, string $marker): int {
        ...
        return preg_match_all('/(?<![\p{L}])' . preg_quote($marker, '/') . '\s*:/iu', $flat);
    }
    ...
    public function generate_lesson_text(array $criteria, string $prompt): string {
        ...
        foreach ($missing as $marker => $min) {
            $wanted[] = $marker === self::MEMO_MARKER
                ? 'не меньше ' . $min . ' строк «' . $marker . ': ...», в каждой - главное понятие '
                  . 'своего раздела короткой фразой'
                : 'не меньше ' . $min . ' строк «' . $marker . ': ...», в каждой - короткий пример '
                  . 'из реальной жизни к своему разделу';
        }
        try {
