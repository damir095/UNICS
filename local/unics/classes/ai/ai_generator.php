...
class ai_generator {
    ...
    public function build_criteria(array $profile): array {
        ...
        $ovz_instructions = [
            ...
            3 => 'Любой порядок действий давай пронумерованными строками «' . self::STEP_MARKER
               . ' 1. ...», «' . self::STEP_MARKER . ' 2. ...» - не меньше '
               . self::STEP_MIN . ' шагов. Не требуй от учащегося быстрых действий.',
            ...
            // против 30.0% в контроле. Требование переписано так, чтобы его можно было
            // посчитать: пояснение обязано стоять СРАЗУ за термином.
    ...
    /**
     * Нужны ли тексту пронумерованные шаги - только НОДА (вид 3).
     */
    public function steps_required(array $criteria): bool {
        return in_array(3, $criteria['ovz_type_ids'] ?? [], true);
    }
    ...
    public function required_artifacts(array $criteria): array {
        ...
        if ($this->steps_required($criteria)) {
            $need[self::STEP_MARKER] = self::STEP_MIN;
        }
        return $need;
    }
    ...
    public function artifact_count(string $text, string $marker): int {
        ...
        //
        // Хвост у шага СВОЙ: он пишется «Шаг 1.», с номером и точкой, а не с двоеточием. Общий
        // шаблон «метка плюс двоеточие» не нашел бы его НИКОГДА, и каждый комплект для НОДА
        // платил бы лишней генерацией, не проходя проверку ни при каком ответе модели.
        $tail = $marker === self::STEP_MARKER ? '\s*\d+\s*[.):]' : '\s*:';
    ...
    public function generate_lesson_text(array $criteria, string $prompt): string {
        ...
        foreach ($missing as $marker => $min) {
            ...
            $wanted[] = match ($marker) {
                self::MEMO_MARKER    => 'не меньше ' . $min . ' строк «' . $marker . ': ...», '
                    . 'в каждой - главное понятие своего раздела короткой фразой',
                self::EXAMPLE_MARKER => 'не меньше ' . $min . ' строк «' . $marker . ': ...», '
                    . 'в каждой - короткий пример из реальной жизни к своему разделу',
                self::STEP_MARKER    => 'не меньше ' . $min . ' пронумерованных строк «'
                    . $marker . ' 1. ...», «' . $marker . ' 2. ...» с порядком действий',
                // Сюда попадать не должно: у каждого артефакта своя формулировка, и тест
                // сторожит, что их число совпадает с числом артефактов. Ветка есть потому, что
                // match без нее бросает UnhandledMatchError ВНЕ try - то есть забытая формулировка
                // роняла бы генерацию урока целиком, а не портила бы один переспрос.
                default              => 'не меньше ' . $min . ' строк «' . $marker . ': ...»',
            };
        }
        try {
