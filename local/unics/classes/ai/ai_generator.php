...
class ai_generator {
    ...
    public function build_criteria(array $profile): array {
        ...
        if (in_array(1, $category_ids, true)) {
            // ЗПР и «иное нарушение здоровья»: перегрузка объемом сводит на нет и короткие
            // абзацы, и повтор ключевых понятий.
            if (array_intersect([4, 6], $valid_ovz_types)) {
                $limits[] = '400–600';
            }
        }
        usort($limits, static fn(string $a, string $b): int => $ranks[$a] <=> $ranks[$b]);
        $word_count = $limits[0];
