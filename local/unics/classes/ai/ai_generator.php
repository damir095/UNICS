...
class ai_generator {
    ...
    /**
     * Сколько вопросов просить СВЕРХ заказанного ([[quiz-buffer-design]]).
     *
     * Живая генерация 2026-08-26: из двух заказов по пять вопросов вышло четыре и три - один снял
     * судья, два не прошли по битому индексу ключа. У модели просили ровно нужное число, а ярусов
     * отбраковки уже четыре, и каждый новый укорачивает тест. Запас в два вопроса покрывает
     * обычный случай; наружу по-прежнему уходит не больше $num - обрезка стоит в конце разбора.
     */
    public const QUIZ_BUFFER = 2;

    /**
     * Потолок токенов на один вопрос: ответ на семь вопросов длиннее, чем на пять.
     *
     * Значение НЕ должно быть меньше прежнего бюджета на вопрос, иначе правка ухудшает то, что
     * лечит: жесткие 4096 на пять вопросов давали 819 токенов на вопрос, и первая редакция с 700
     * оставляла на вопрос МЕНЬШЕ прежнего (найдено ревью 2026-08-26).
     */
    private const QUIZ_TOKENS_PER_QUESTION = 900;
    ...
    public function build_criteria(array $profile): array {
        ...
        $ovz_instructions = [
            ...
            // Замер 2026-09-02 ([[ovz-adaptation-measured]]): прежняя редакция не меняла выход
            // ВОВСЕ. «Короткие абзацы» контроль выполнял и без указания (2.7 предложения), а
            // единственное невыполненное требование - «повторяй ключевые понятия» - было
            // непроверяемым пожеланием, и повтор в плече ЗПР выходил даже НИЖЕ контроля.
            // Поэтому требование заменено на артефакт, наличие которого можно посчитать.
            4 => 'Очень короткие абзацы (2–3 предложения). Пошаговая структура обязательна. '
               . 'После каждого раздела ставь отдельной строкой «Запомни: ...» и повторяй в ней '
        ...
        $ovz_item_instructions = [
            ...
            // Про время тут не пишем: таймер - настройка теста, которую ставит course_builder, а
            // не свойство текста вопроса. Модель либо игнорировала бы это требование, либо
            // вписала бы «не ограничено по времени» в сам вопрос (найдено ревью 2026-08-25).
            3 => 'Не строй вопрос на действии с предметом или на скорости выполнения. Варианты '
               . 'делай короткими и заметно разными, чтобы выбор не требовал долгого перебора и '
               . 'перечитывания.',
        ...
        //
        // Вид ОВЗ в этом минимуме раньше НЕ участвовал, и ребенок с ЗПР при высоком балле
        // получал 600-800 слов - вместе с указанием «очень короткие абзацы, повторяй ключевые
        // понятия» ([[criteria-conflicts-design]]). Утомляемость и объем удерживаемого текста -
        // это про нарушение, а не про уровень подготовки, поэтому потолок ставит и оно.
        $ranks = ['250–350' => 0, '300–400' => 1, '400–600' => 2, '600–800' => 3];
        $limits = [match ($eff_level) {
            1 => '300–400',
            3 => '600–800',
            default => '400–600',
        }];
        if (in_array(3, $category_ids, true)) {
            $limits[] = '250–350'; // длительное лечение - короткие модули
        }
        // ЗПР и «иное нарушение здоровья»: перегрузка объемом сводит на нет и короткие абзацы,
        // и повтор ключевых понятий.
        //
        // Гейта по категории 1 здесь НЕТ намеренно: sync_student_taxonomies() пишет виды ОВЗ и
        // категории независимо, и через импорт достижимо состояние «вид ОВЗ записан, категории
        // нет». Форма такое состояние не создает (edit_user.php выводит категорию из наличия
        // видов), а импорт берет колонки порознь - и ребенок получал бы 600-800 слов при
        // записанной ЗПР (найдено ревью 2026-08-25). Раз вид нарушения записан, адаптация
        // применяется.
        if (array_intersect([4, 6], $valid_ovz_types)) {
            $limits[] = '400–600';
        }
        // Минимум берем напрямую: usort по $ranks[$a] требовал, чтобы КАЖДАЯ строка лимита
        // нашлась в карте рангов, а список и карта - две копии одного перечня, связанные
        // побайтовым совпадением строк с длинным тире. Новый диапазон в одном списке дал бы
        // предупреждение и null, который сравнением сходит за самый жесткий лимит.
        $word_count = $limits[0];
        foreach ($limits as $l) {
            if (($ranks[$l] ?? PHP_INT_MAX) < ($ranks[$word_count] ?? PHP_INT_MAX)) {
                $word_count = $l;
            }
        }
        ...
        // Указания по видам ОВЗ идут по НАЛИЧИЮ ВИДА, а не по категории: категории и виды
        // пишутся независимо (sync_student_taxonomies), и через импорт достижимо состояние
        // «вид записан, категории нет». В нем превью печатало ребенку диагноз, а адаптации не
        // было вовсе (найдено ревью 2026-08-25). Общее указание по категории остается для
        // случая, когда категория ОВЗ проставлена, а конкретный вид нет.
        ...
        } else if (in_array(1, $category_ids, true)) {
        ...
        if (in_array(3, $category_ids, true)) {
            // Времени чтения тут больше нет: «10-15 минут» спорило с заданным объемом (при
            // 250-350 словах это 2-3 минуты), и два разных числа про одно и то же модель мирила
            // наугад ([[criteria-conflicts-design]]). Объем задается отдельной строкой промта.
            $special_parts[] = 'Учащийся на длительном лечении. Текст должен читаться за один '
                . 'короткий подход, без длинных отступлений. Завершай текст коротким '
                . 'мотивирующим выводом.';
        ...
        if (in_array(4, $category_ids, true)) {
            // Категория задает ТИП познавательной задачи, а не ее сложность
            // ([[criteria-conflicts-design]]). Сложностью командует уровень - промт учебного
            // текста прямо говорит «Сложность строго соответствует уровню», - и прежнее «добавь
            // углублённые факты» лезло в ту же ось: у одаренного ребенка с баллом ниже 50%
            // уровень понижался, и рядом вставали «углубляй» и «делай проще базового».
            //
            // Педагогически это и вернее: низкий балл у одаренного ребенка - сигнал о потере
            // интереса или пробеле, а не о неспособности. Ему нужен доступный по сложности
            // материал с исследовательским разворотом, а не более трудный.
            $special_parts[] = 'Учащийся одарённый. Дай нестандартный угол зрения на тему, '
                . 'покажи связи с другими темами и закончи исследовательским вопросом.';
    ...
    public function adaptation_block(array $criteria, string $kind = self::BLOCK_TEXT): string {
        // Оба несоответствия падают ГРОМКО. Прежний код молча откатывался на текстовый набор при
        // любом $kind кроме точного BLOCK_ITEMS и молча выдавал пустоту при отсутствующем ключе -
        // то есть возвращал ровно тот дефект, который эта развилка и чинит, не роняя ни одного
        // теста (найдено ревью 2026-08-25).
        $items = match ($kind) {
            self::BLOCK_TEXT  => false,
            self::BLOCK_ITEMS => true,
            default => throw new \coding_exception('Неизвестный вид блока адаптации: ' . $kind),
        };
        $key = $items ? 'special_parts_items' : 'special_parts';
        if (!array_key_exists($key, $criteria)) {
            throw new \coding_exception("В критериях нет ключа {$key}: адаптация потерялась бы молча");
        }
        $parts = $criteria[$key];
    ...
    /**
     * Артефакт, обязательный в учебном тексте для ребенка с ЗПР ([[ovz-adaptation-measured]]).
     *
     * Замер показал, что прежнее указание «повторяй ключевые понятия несколько раз» выход не
     * меняло ВОВСЕ: повтор в плече ЗПР выходил даже ниже контроля. Непроверяемое пожелание
     * модель игнорирует молча, и адаптация существовала только на бумаге. Требование заменено на
     * артефакт, наличие которого считается регулярным выражением, - и тогда его можно не только
     * попросить, но и проверить.
     */
    public const MEMO_MARKER = 'Запомни';

    /**
     * Нужен ли учебному тексту артефакт «Запомни:».
     *
     * Только ЗПР (вид 4): именно там барьером стоит объем рабочей памяти, и повтор главного после
     * каждого раздела снимает его. Прочим видам нарушения артефакт не нужен, а лишняя строка
     * ломала бы предсказуемую структуру, которой требует РАС.
     */
    public function memo_required(array $criteria): bool {
        return in_array(4, $criteria['ovz_type_ids'] ?? [], true);
    }

    public function has_memo(string $text): bool {
        return (bool)preg_match('/' . self::MEMO_MARKER . '\s*:/u', $text);
    }

    /**
     * Учебный текст с проверкой обязательного артефакта и ОДНИМ переспросом.
     *
     * Живая проба дала 5 из 5 соблюдений с первого раза, так что переспрос - предохранитель от
     * молчаливого отката (смена модели, правка промта), а не рабочий режим. Второй заход стоит
     * токенов, поэтому он один.
     *
     * Отказ переспроса НЕ теряет первый текст: урок ребенку важнее артефакта в нем.
     */
    public function generate_lesson_text(array $profile, string $prompt): string {
        $text = $this->generate_text($prompt);
        if (!$this->memo_required($this->build_criteria($profile)) || $this->has_memo($text)) {
            return $text;
        }
        try {
            $retry = $this->generate_text($prompt . PHP_EOL . PHP_EOL
        ...
        return $this->has_memo($retry) ? $retry : $text;
    }
...
- Для формул и математических выражений используй нотацию \(...\) (например: \(x^2 + y^2\)), НЕ используй знак доллара \$";
    ...
    public function generate_quiz(array $profile, string $topic, string $source_text = '',
        ...
        // Просим с запасом, отдаем ровно заказанное ([[quiz-buffer-design]]). Обрезка до $num
        // стоит в конце questions_from_reply - там же, где отсекается лишнее, присланное
        // моделью сверх просьбы.
        $ask = $num + self::QUIZ_BUFFER;

        $prompt = "Ты - педагог, составляющий тестовые задания для российских школьников.

Составь ровно {$ask} вопросов с множественным выбором по теме «{$topic}» для ученика {$class_num} класса (уровень: {$level}).{$src}
...
- Все варианты примерно одной длины. Верный ответ НЕ должен быть заметно длиннее остальных
...
{\"questions\":[{\"text\":\"Текст вопроса?\",\"answers\":[\"Вариант А\",\"Вариант Б\",\"Вариант В\",\"Вариант Г\"],\"answer\":\"Вариант Б\",\"correct\":1,\"solution\":\"вычисление или пусто\"}]}
answer - ДОСЛОВНЫЙ ТЕКСТ правильного варианта, скопированный из списка answers знак в знак. НЕ номер и НЕ пересказ.
correct - номер того же варианта, считая С НУЛЯ (0, 1, 2 или 3). Оба поля обязательны и должны указывать на ОДИН вариант.";
        ...
        for ($attempt = 1; $attempt <= self::QUIZ_PARSE_ATTEMPTS; $attempt++) {
            // Потолок токенов растет вместе с заказом: на жестких 4096 ответ про семь вопросов
            // обрезался бы, и лечение обернулось бы той же болезнью.
    ...
    private function questions_from_reply(string $raw, int $num, array &$surplus = []): array {
        ...
        foreach ((array)($data['questions'] ?? []) as $q) {
            ...
            // Ключ берем ТЕКСТОМ, а не номером ([[key-as-text-design]]). У номера нет способа
            // проверки: сбитый на единицу он остается законным числом, и обе беды 27 августа -
            // «correct = 4 при четырех вариантах» и сбитый ключ внутри диапазона - росли отсюда.
            // Текст же мы сопоставляем сами, то есть индекс ВЫЧИСЛЯЕТСЯ, а не принимается на веру.
            $bytext = false;
            $byindexq = false;
            $correct = null;
            // Только СТРОКА. Число в этом поле - не текст варианта, а сбившийся на старый контракт
            // номер: при вариантах «1», «2», «3» оно совпало бы с чужим вариантом дословно, и
            // подделка выглядела бы проверенной (найдено ревью). Нескалярное значение к тому же
            // дало бы литерал «Array» - гейт is_string закрывает и это.
            $wanted = is_string($q['answer'] ?? null) ? trim($q['answer']) : '';
            if ($wanted !== '') {
                // Варианты нормализуем ТАК ЖЕ, как question_sanity: иначе наш «дубль» и ее
                // «дубль» - разные множества, и вопрос, который мы сочли неоднозначным, она
                // пропустила бы дальше. Приставку с номером срезаем только у ответа модели.
                $norm = array_map([question_sanity::class, 'normalize'], $answers);
                $want = question_sanity::option_text(output_style::strip_math_markup($wanted));
                $at = array_search($want, $norm, true);
                // Дубль среди вариантов делает сопоставление неоднозначным: какой из двух
                // одинаковых текстов имела в виду модель, неизвестно.
                if ($at !== false && count(array_keys($norm, $want, true)) === 1) {
                    $correct = (int)$at;
                    $bytext = true;
                }
            }

            // Запасной путь: модель не прислала текст или пересказала его своими словами.
            // Индекс ключа при этом НЕ зажимаем - раньше correct = 7 при четырех вариантах молча
            // объявлял верным последний, и ребенок получал «неверно» за верный ответ.
            if ($correct === null) {
                if (!isset($q['correct']) || !is_numeric($q['correct'])) {
                    // Ключа нет НИ ОДНОГО из двух способов - выдумывать его нельзя. Первая
                    // редакция этой правки убрала correct из промта и оставила `?? 0`: вопрос
                    // уезжал ребенку и в общий пул с верным вариантом «первый по списку»
                    // (найдено ревью).
                    $dropped++;
                    $this->trace('  Ключа нет: текст не совпал с вариантами, номер не прислан');
                    continue;
                }
            ...
            // Единственное исключение: индекс, равный ЧИСЛУ вариантов, - это счет от единицы, а
            // не потеря соответствия. Проба 2026-08-27 на 42 вопросах: все пять битых индексов
            // оказались одного вида, `correct = 4` при четырех вариантах, то есть «четвертый
            // вариант» ([[one-based-key-design]]). Мы выбрасывали каждый восьмой годный вопрос.
            //
            // Это НЕ возврат к зажиму: сдвигается ровно однозначный случай, все остальное вне
            // диапазона по-прежнему выбывает, а сдвинутый вопрос все равно идет к слепому судье.
            // Гейта «только для номера» тут нет: ключ, найденный сопоставлением, - это индекс
            // существующего варианта, он строго меньше их числа, и условие ниже на нем не
            // срабатывает в принципе. Лишнее условие выглядело бы предохранителем, которого
            // нечем проверить (найдено ревью: тест на него был тавтологией).
            $onebased = false;
            ...
            $survived[] = ['text' => $text, 'answers' => $answers, 'correct' => $correct,
                ...
                // Сдвиг ключа считаем ОТДЕЛЬНО от арифметической правки: иначе по логу не
                // отличить «модель перестала считать с единицы» от «сдвиг молча вывозит
                // каждый восьмой вопрос», а именно этим числом мерилась задача (найдено ревью).
                'onebased' => $onebased,
                'byindex' => !empty($byindexq),
                // Подозрения держим ПРИ ВОПРОСЕ, а не в общем списке: с запасом
                // ([[quiz-buffer-design]]) часть разобранных вопросов заведомо не дойдет до
                // ребенка, и педагог читал бы отчет про задания, которых нет. Тот же дефект
                // уже чинили для счетчика исправленных ключей (найдено ревью 2026-08-26).
                'notes' => $signs['notes']];
        ...
        [$kept, $byjudge, $byunverified] = $this->judge_survivors($survived);
        ...
        foreach ($kept as $q) {
            ...
            // Считаем только то, что ДОШЛО: с запасом разбирается больше вопросов, чем нужно, и
            // счетчик по всем разобранным врал бы вверх - тот же дефект уже чинили для ключей и
            // для пометок (найдено ревью).
            if (!empty($q['byindex'])) {
                $byindex++;
            }
            unset($q['wasfixed'], $q['notes'], $q['onebased'], $q['byindex']);
            ...
            if (count($result) >= $num) {
                // Потолок запаса: модель иногда шлет вопросов вдвое больше просимого, а каждый
                // излишек - это записи в банке вопросов, привязка к элементу и строка уровня.
                // Пока запас выбрасывался, многословие модели ничего не стоило; теперь стоит.
                if (count($surplus) >= self::QUIZ_BUFFER) {
                    continue;
                }
                $surplus[] = $q;
                // Про запас педагогу говорим ОТДЕЛЬНО, но говорим: эти вопросы уходят в общий
                // пул и достанутся другим детям, так что молчать про починенный ключ и про
                // подозрения тут уже нельзя. Пока запас выбрасывался, молчание было верным
                // (найдено ревью: правка пережила отмену своей же предпосылки).
                if ($wasfixed) {
                    $surplusfixed++;
                }
        ...
        if ($surplus) {
            $this->trace('  Запас: годных вопросов ушло в пул ' . count($surplus)
                . ($surplusfixed ? ' (из них с исправленным ключом ' . $surplusfixed . ')' : ''));
        }
        if ($shifted) {
            $this->trace('  Ключей, посчитанных моделью с единицы: ' . $shifted);
        }
        // Сколько ключей пришлось брать номером вместо текста. Ноль - здоровое состояние; рост
        // означает, что модель перестала копировать вариант дословно, и хрупкий путь ожил.
        if ($byindex) {
            $this->trace('  Ключ взят номером (текст не совпал с вариантами): ' . $byindex);
        }

        if ($fixed || $dropped || $bysigns || $byjudge || $byunverified) {
            $this->trace('  Проверка заданий: исправлено ключей ' . $fixed
                . ', отброшено арифметикой ' . $dropped
                . ', признаками ' . $bysigns
                . ', судьей ' . $byjudge
                . ', сдвигом без проверки ' . $byunverified);
        }
        if ($notes) {
     ...
     *               сколько отброшено сдвигом ключа, который некому было проверить]
     */
    private function judge_survivors(array $survived): array {
        ...
        $byjudge = 0;
        $byunverified = 0;
        // Состоялась ли проверка судьей ВООБЩЕ. Пустые вердикты неотличимы от «судья со всем
        // согласился», а для сдвинутого ключа эта разница решающая.
        $judged = false;
        ...
        // Считаем спорные вопросы, у которых выбор судьи лег ровно на вариант ПЕРЕД ключом.
        // Это признак сбитого на единицу ключа - тот же счет с единицы, что и в разборе, только
        // внутри диапазона, где по значению его не видно ([[judge-key-shift-design]]).
        //
        // ТОЛЬКО СЧИТАЕМ. Ключ по мнению судьи не переносится: вне математики судья не авторитет,
        // и его расхождение - сигнал спорности задания, а не установление истины (см. докблок
        // answer_judge). Правку «переставить ключ по такому признаку» пробовали 2026-08-27 и
        // откатили: выигрыш вышел два вопроса из сорока, а цена ошибки - «неверно» за верный
        // ответ у ребенка и порча калибровки через общий пул.
        // Отдельной проверки «судья работал» тут нет намеренно: на всех ветках отказа вердиктов
        // не остается вовсе, а `at` обнуляется в самом answer_judge. Гейт был бы недостижим, а
        // недостижимый предохранитель нечем проверить - и он тихо гниет.
        $shiftlike = 0;
        foreach ($survived as $i => $q) {
            ...
            $at = $at_list[$i] ?? null;
            $key = (int)($q['correct'] ?? 0);
            if ($at !== null && $key > 0 && $at === $key - 1) {
                $shiftlike++;
            }
        }
        if ($shiftlike) {
            $this->trace('  Похоже на сбитый на единицу ключ: ' . $shiftlike
                . ' (вопросы отброшены; ключ по мнению судьи не переносим)');
        }
        ...
        foreach ($survived as $i => $q) {
            ...
            // Сдвиг ключа со счета от единицы - ДОГАДКА, и держится она на том, что вопрос
            // кто-то перепроверил. Расчет молчит на неарифметических заданиях, а при отказе
            // судьи вердиктов нет вовсе - и тогда мы отдали бы ребенку ключ, который сами же и
            // подвинули. До правки такой вопрос выбывал целиком, так что откат к отбраковке -
            // не потеря, а возврат прежнего поведения (найдено ревью).
            if (!empty($q['onebased']) && empty($q['computed']) && !$judged) {
                $byunverified++;
                continue;
            }
            unset($q['computed']);
        ...
        return [$kept, $byjudge, $byunverified];
    }
...
Верни только текст задания. Без заголовков, без вводных слов - только само задание.";
    ...
    public function generate_video_script(array $profile, string $topic,
        ...
        $class_num = $profile['class_number'] ?? 5;
        $level     = $c['level_label'];
        // Указания ОВЗ идут ТЕКСТОВЫМ набором намеренно: для видеоурока они уместнее, чем для
        // страницы («текст должен хорошо восприниматься на слух» - это ровно про видео). Третий
        // набор под слайды НЕ заводился: проверка показала, что текстовые указания тут не мешают.
        //
        // А вот ЧИСЛО пришлось согласовать. Промт требовал «3-4 предложения», указание для ЗПР -
        // «2-3»: два разных числа про одно и то же, тот самый класс дефекта, что уже чинили в
        // критериях. Замер 2026-08-31 на двадцати слайдах показал, что не соблюдается НИ ОДНО из
        // них: модель пишет около двух предложений и с ОВЗ (в среднем 2.0), и без него (1.9). То
        // есть указание ОВЗ не проигрывало общему правилу - не работало ни одно. Число приведено
        // к наблюдаемому и к более строгому указанию, чтобы промт хотя бы не спорил сам с собой.
        $block     = $this->adaptation_block($c);
        $extra     = $this->extra_block($extra_context);
...
- content: 2-3 коротких предложения, доступный язык для {$class_num} класса, уровень «{$level}»
