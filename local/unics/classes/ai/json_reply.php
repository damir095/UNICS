...
class json_reply {
    ...
    public static function decode(string $raw, string $expect_key = ''): ?array {
        ...
        // незачем.
        //
        // Область резерва - именно СПИСОК: либо корень ответа, либо значение ожидаемого
        // ключа, ограниченное его закрывающей скобкой. Одиночный объект с чужим ключом
        // («questions» там, где ждали «sections») списком не является и за ответ не сойдет.
        if ($expect_key !== '') {
    ...
    private static function list_of_key(string $raw, string $key): string {
        // Скобка должна стоять СРАЗУ за ключом: иначе «"questions":"нет вопросов","other":[...]»
        // отдавал бы чужой список под нашим ключом (найдено ревью 2026-08-21).
        if (!preg_match('~"' . preg_quote($key, '~') . '"\s*:\s*\[~u', $raw, $m, PREG_OFFSET_CAPTURE)) {
            return '';
        }
        $bracket = $m[0][1] + strlen($m[0][0]) - 1;
        // Хвост за концом списка тоже не наш: без границы запасной разбор собирал объекты из
        // соседних ключей («meta») и выдавал их за вопросы.
        $end = self::balanced_end($raw, $bracket);
        return $end === null
            ? substr($raw, $bracket)
            : substr($raw, $bracket, $end - $bracket + 1);
    }
