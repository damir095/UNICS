...
class arithmetic_checker {
    ...
    public static function verdict(string $text, array $answers, int $correct,
        ...
        // Порядок источников: сперва то, что можно посчитать по САМОМУ вопросу, и только потом
        // решение модели. Оно не независимо от ключа: живая генерация 2026-08-22 дала задание,
        // где модель ошиблась в решении ровно так же, как в ключе («5/20 + 20/20 = 25/20»), и
        // тем самым подтвердила собственную ошибку - вердикт был «сошлось» вместо «отбросить».
