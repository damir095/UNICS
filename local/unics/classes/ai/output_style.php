...
class output_style {
    ...
    private static function hard_break_lines(string $text): array {
        ...
        if (count($parts) === 1 && $parts[0] === $text && preg_last_error() !== PREG_NO_ERROR) {
            // Разбор не удался (битый UTF-8). Индексы одного разбиения не сойдутся с индексами
            // другого, и приписка ушла бы не в ту строку - лучше не трогать вовсе. Тот же прием
            // стоит в question_sanity::normalize().
            return [];
        }
        ...
        foreach ($parts as $i => $part) {
            if ($i % 2 === 0 && preg_match('/^\s*```/u', $part)) {
                // Внутрь блока кода не пишем: там пробелы значимы, и весь класс их не трогает
                // ([[code-fence-and-math-design]]). Найдено ревью: приписка лезла и туда.
                $infence = !$infence;
                continue;
            }
            if ($infence) {
    ...
    private static function restore_hard_breaks(string $text, array $hard): string {
        ...
        foreach ($parts as $i => $part) {
            if (!empty($hard[$i]) && trim($part) !== '') {
                // Сперва СРЕЗАЕМ хвост, потом ставим ровно два. Обрезка пробелов в конце
                // строки ловит их только перед одиночным переводом строки: перед парой
                // CR+LF они оставались, и приписка дала бы три пробела вместо двух.
                //
                // Хватает побайтного среза: схлопывание выше уже перевело любые пары
                // «широких» пробелов (в том числе неразрывный) в обычный, так что сюда
                // они не доходят. Проверять регуляркой было бы нечего - мутация на такую
                // проверку не падает, и предохранитель оказался бы недостижимым.
                $parts[$i] = rtrim($part, " \t") . '  ';
            }
        }
    ...
    private static function clean_math(string $text): string {
        ...
        // Схлопываем пробелы, оставшиеся от снятых разделителей - но ТОЛЬКО в середине
        // строки. Пара пробелов в конце строки это жесткий перенос markdown, и съедать его
        // нельзя.
        //
        // Раньше тут стояло восстановление по номерам строк, как в clean(). Оно негодно:
        // чистка внутри map_outside_code() подрезает пустые строки у ограждений блока кода,
        // число строк меняется, и перенос уезжал на СОСЕДНЮЮ строку. Проверено: в тексте с
        // блоком кода из трех переносов доживал один, и тот не на своей строке (найдено
        // ревью).
        $out = preg_replace('/[ \t]{2,}(?=\S)/u', ' ', $out) ?? $out;
        return trim($out);
    }
