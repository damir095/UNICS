...
class codifier_analytics {
    ...
    /**
     * Наименьший предел заданий, при котором порог $se ДОСТИЖИМ.
     *
     * Ровно на поле остановки не происходит: и сервис, и estimate_precision::is_provisional
     * держат границу строго - «se СТРОГО меньше порога». Прямая инверсия формулы дает число, где
     * ошибка РАВНА порогу, поэтому совет «увеличьте предел до N» приводил ровно в отвергаемую
     * точку: при пороге 0.5 он называл 12, а предел 12 дает ровно 0.5, и проверка оставалась в
     * тревоге. Из четырех проверенных порогов совет не работал на трех (найдено ревью).
     *
     * Отсюда +1 задание сверх инверсии. И результат ПЕРЕПРОВЕРЯЕТСЯ той же функцией, которой
     * судит вызывающий: решать, сработает совет или нет, дробной арифметике нельзя.
     *
     * @param float $se желаемый порог стандартной ошибки
     * @return int предел заданий за сессию, при котором rasch_floor() строго меньше $se
     */
    public static function items_for_se(float $se): int {
        if ($se <= 0.0) {
            // Ошибка неотрицательна: ни один предел не даст меньше нуля, и советовать нечего.
            return 0;
        }
        $n = max(1, (int)floor((1.0 / ($se * $se) - 1.0) / self::ITEM_INFO_BEST) + 1);
        // Дробный разряд может отправить floor() на соседнее целое. Досчитываем по самой формуле,
        // а не по алгебре: пол убывает с каждым заданием, дальше первого шага цикл не уходит.
        for ($i = 0; $i < 4 && self::rasch_floor($n) >= $se; $i++) {
            $n++;
        }
        return $n;
    }
    ...
    /**
     * Наименьший порог точности, ДОСТИЖИМЫЙ при пределе в $cap заданий - в сотых, то есть в том
     * виде, в каком его вводит администратор.
     *
     * Округление именно ВВЕРХ. format_float($floor, 2) округляет по правилам арифметики, и совет
     * «поднимите порог до X» уводил НИЖЕ пола: из шести проверенных пределов он работал на одном,
     * и только потому, что 0.40825 округлилось вверх само (найдено ревью). Плюс сотая там, где пол
     * попадает в круглое число: ровно на поле остановки не происходит.
     */
    public static function threshold_for_items(int $cap): float {
        $floor = self::rasch_floor($cap);
        $advice = ceil($floor * 100.0) / 100.0;
        return $advice > $floor ? $advice : $advice + 0.01;
    }
    ...
    public static function attainable_se(array $discriminations, int $cap): float {
        // Ноль означает «заданий не дадут», а НЕ «предела нет». Прежняя редакция при $cap <= 0
        // считала точность по ВСЕМУ пулу, то есть обещала тем больше, чем меньше предел, - а то же
        // самое число в rasch_floor() значило ровно обратное, голый априор (найдено ревью). Смысл
        // у нуля один на оба метода: нет заданий - нет и уточнения априора.
        if ($cap <= 0) {
            return 1.0;
        }
        // Множитель тот же ITEM_INFO_BEST, а не своя четверка: копия константы отстает от правок.
        $info = array_map(
            static fn(float $a): float => ($a * $a) * self::ITEM_INFO_BEST, $discriminations);
        rsort($info);
        return 1.0 / sqrt(1.0 + array_sum(array_slice($info, 0, $cap)));
    }
    ...
    public static function element_bank_readiness(int $codifier_id): array {
        ...
        $minitems = \local_unics\learning\cat_limits::min_items();

        // Тот же предел, по которому идет сама сессия (cat_limits зовет и cat_session_manager).
        // Без запаса незаданная
        ...
        $maxitems = \local_unics\learning\cat_limits::max_items();
