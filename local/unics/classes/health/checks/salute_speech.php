...
class salute_speech implements check {
    ...
    public function run(): check_result {
        ...
        if (!tts_status::is_unavailable()) {
            // Осторожно с формулировкой: метка помнит ТОЛЬКО отказы, следа удачи в системе нет.
            // «Работала при последней попытке» на свежей установке было бы ложным успокоением -
            // попытки могло не быть ни одной, а пакет при этом не оплачен (найдено ревью).
            return check_result::ok('Отказов не зафиксировано',
                ['Как узнается' => 'из реальных генераций; отдельной проверки нет']);
        }
