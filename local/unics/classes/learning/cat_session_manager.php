...
class cat_session_manager {
    ...
    private static function config(): array {
        // Каждое из трех значений берется ТАМ ЖЕ, где его читают остальные: порог - из
        // estimate_precision (им же меряется честность результата), пределы - из cat_limits (по
        // ним же индикатор готовности и проверка здоровья считают достижимую точность). Свои
        // копии означали бы, что сессия останавливается по одной планке, а методисту обещают
        // другую; здесь публичные константы 20 и 5 стояли рядом с литералами 20 и 5, то есть
        // расходились внутри одного класса (найдено ревью).
        return ['se' => \local_unics\adaptive\estimate_precision::threshold(),
                'min' => cat_limits::min_items(), 'max' => cat_limits::max_items()];
    }
