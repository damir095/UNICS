...
class item_irt_manager {
    ...
    /**
     * С какого числа наблюдений калибровку МОЖНО ИСПОЛЬЗОВАТЬ. ЕДИНСТВЕННЫЙ источник порога: им
     * пользуются пул заданий, адаптивная проверка и индикатор готовности к CAT.
     *
     * Слово «достоверная» из первой строки убрано намеренно - оно противоречило замеру ниже в
     * этом же докблоке (найдено ревью). Порог отсекает вырожденные оценки, а не гарантирует
     * точность.
     *
     ...
     * по этой же вырожденной трудности, потому что брала все строки параметров подряд.
     *
     * ЧТО ЭТОТ ПОРОГ НА САМОМ ДЕЛЕ ЗНАЧИТ (замер 2026-09-07, [[irt-calibration-precision]]).
     * Десять - это «один класс прошел тест», то есть довод УДОБСТВА, а не точности. Точность при
     * десяти ответах: СКО оценки трудности 0.99 логита, и у 65 процентов заданий ошибка больше
     * половины логита. Половина логита - это заметная разница трудности, а не мелочь.
     *
     * Для СКО 0.5 нужно около тридцати ответов, для 0.3 - около ста. Пилот на шестнадцати детях
     * столько не дает, поэтому порог оставлен рабочим, но называть его «достоверностью» нельзя:
     * он отсекает вырожденные оценки, а не гарантирует точность. Где нужна точная трудность -
     * смотреть на n, а не на факт калибровки.
     */
     ...
     * сознательное: методисту надо показывать, скольких УЧАЩИХСЯ еще не хватает, - иначе колонка
     * «2PL» стоит нулем без объяснения.
     *
     * Прежде здесь стояло «сервис своих порогов наружу не отдает», и копия держалась на одной
     * лишь просьбе править оба места. Она разъехалась при первом же изменении, поэтому сервис
     * теперь отдает пороги в /health, а проверка здоровья сверяет их с этой константой и
     * докладывает расхождение. Править по-прежнему надо оба места - но молча разойтись они уже
     * не могут.
     *
     * Было 20 - число ниоткуда. Замер 2026-09-07 ([[irt-calibration-precision]]) сравнил ОБЕ
     * модели на ОДНИХ данных: даже когда дискриминация действительно разная, 2PL начинает
     * выигрывать у Раша лишь около двухсот ответов, а при 20-60 расходился - оценка дискриминации
     * упиралась в потолок в 17-42 процентах прогонов. Порог поднят до измеренного.
     *
     * Практическое следствие честное и неудобное: на пилоте из шестнадцати детей 2PL не включится
     * никогда. Это и есть настоящее положение дел, а не повод оставить порог пониже.
     */
    const MIN_N_FOR_2PL = 200;
