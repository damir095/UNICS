...
class activity_meta {
     ...
     */
    public static function set_flags(int $cmid, ?bool $is_final = null, ?bool $is_milestone = null): void {
        global $DB;
        $now = time();
        $rec = self::get($cmid);
        if (!$rec) {
            // Нечего хранить, если ни один флаг не выставляется в 1.
            if (!$is_final && !$is_milestone) {
                return;
            }
        ...
        // Оба флага сняты — метаданные больше не несут смысла, удаляем строку.
        if ((int)$rec->is_final === 0 && (int)$rec->is_milestone === 0) {
