...
if ($hassiteconfig) {
    ...
    // Достижимый минимум ошибки при нынешнем пределе вопросов. Точность ограничена НЕ размером
    // банка, а числом вопросов, которое ребенок успевает получить: больше предела ему не дадут,
    // сколько заданий ни накопи. Отсюда SE = 1/sqrt(1 + предел/4) при дискриминации 1
    // ([[cat-attainable-precision]]).
    //
    // Число показывается ПРЯМО ЗДЕСЬ, потому что здесь его и вводят. Замер показал, что стоявший
    // порог 0.3 не срабатывал ни разу, и администратору об этом никто не говорил: он видел
    // настройку, у которой нет следствия. Проверка здоровья ловит то же самое, но она - страховка,
    // а место ошибки тут.
    $catmax = \local_unics\learning\cat_limits::max_items();
    $catfloor = \local_unics\codifier_analytics::rasch_floor($catmax);
    // Наименьший порог, который тут СРАБОТАЕТ. Не format_float($catfloor, 2): арифметическое
    // округление уводит НИЖЕ пола, и советованное число оказывается таким же нерабочим, как
    // введенное - из шести проверенных пределов такой совет работал на одном (найдено ревью).
    $catadvice = \local_unics\codifier_analytics::threshold_for_items($catmax);
    $adaptive->add(new admin_setting_configtext(
        ...
        . '(но не раньше минимального числа вопросов). По умолчанию 0.3.<br>'
        . 'При нынешнем пределе в ' . $catmax . ' вопросов точность обычных заданий доходит до '
        . '<strong>' . format_float($catfloor, 3, false) . '</strong>. Этот порог и все, что '
        . 'мельче, не сработают: каждая проверка дойдет до предела с пометкой «предварительная '
        . 'оценка», а накопить больше заданий не поможет - ребенку все равно дадут не больше '
        . 'предела. Наименьший работающий порог тут - <strong>'
        . format_float($catadvice, 2, false) . '</strong>. Оценка осторожная: у заданий с высокой '
        . 'различающей способностью точность бывает выше.',
        '0.3', PARAM_FLOAT
    ));
    $adaptive->add(new admin_setting_configtext(
