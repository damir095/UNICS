...
if ($hassiteconfig) {
    ...
    // а место ошибки тут.
    $catmax = (int)get_config('local_unics', 'cat_max_items');
    if ($catmax <= 0) {
        $catmax = \local_unics\learning\cat_session_manager::DEFAULT_MAX_ITEMS;
    }
    ...
    $adaptive->add(new admin_setting_configtext(
        ...
        . 'При нынешнем пределе в ' . $catmax . ' вопросов точность доходит до '
        . '<strong>' . format_float($catfloor, 3) . '</strong>: порог мельче этого не сработает '
        . 'никогда, и каждая проверка дойдет до предела с пометкой «предварительная оценка». '
        . 'Накопить больше заданий не поможет - ребенку все равно дадут не больше предела.',
        '0.3', PARAM_FLOAT
    ));
    $adaptive->add(new admin_setting_configtext(
