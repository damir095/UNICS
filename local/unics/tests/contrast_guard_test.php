...
final class contrast_guard_test extends \advanced_testcase {
    ...
    /**
     * Пара тинта обязана держать AA во ВСЕХ шестнадцати сочетаниях схем, а акцент - до нее доходить.
     *
     * До 2026-08-28 пара была КОНСТАНТОЙ: персик плюс глубокий оранжевый в любой схеме. Выбранный
     * пользователем акцент не доходил до тонированных контролов - он оставался в одной рамке
     * (найдено ревью). Первая попытка привязки откачена по замерам, вторая (2026-09-01) принята:
     * у каждого акцента свой тинт, свой цвет на нем и свой тон под курсором.
     *
     * Тест судит ТОКЕНЫ, а не то, что видит ребенок. Значение может смениться верно и все равно
     * быть перебито на контроле правилом с большей специфичностью - ровно так и вышло у
     * `.unics-assistant-note` в темной схеме. Доход токена до контрола проверяет живой замер по
     * собранному бандлу; отсюда и осторожная формулировка провала.
     */
    public function test_tint_pair_holds_aa_in_every_scheme(): void {
        $decls = contrast_analyzer::declarations(contrast_analyzer::css());

        $default = contrast_analyzer::tokens($decls, []);
        $defaultfg = $default['--unics-on-tint'] ?? null;
        $defaultbg = $default['--unics-tint-bg'] ?? null;
        $this->assertNotNull($defaultfg, 'Токен --unics-on-tint не разрешается без акцента');
        $this->assertNotNull($defaultbg, 'Токен --unics-tint-bg не разрешается без акцента');
        ...
        foreach (contrast_analyzer::combos() as $combo) {
            ...
            // игнорировала выбранный акцент. Мутация «вернуть константу» первое утверждение не
            // роняла - докблок обещал больше, чем тест проверял. Поэтому при выбранном акценте
            // требуем свои значения ОБЕИХ сторон пары: сторожить только текст мало, удаление
            // пер-акцентного --unics-tint-bg проходило молча.
            if (!array_filter($combo, static fn(string $c): bool => str_contains($c, 'accent-'))) {
                continue;
            }
            // Сравнение без учета регистра: #4A2C75 и #4a2c75 - один цвет, и «отличие» в одном
            // регистре было бы ложным пропуском.
            if (strcasecmp($fg, $defaultfg) === 0) {
                $ignored[] = sprintf('  %s: --unics-on-tint', $label);
            }
            if (strcasecmp($bg, $defaultbg) === 0) {
                $ignored[] = sprintf('  %s: --unics-tint-bg', $label);
            }
        }
...
", $bad)
        ...
        $this->assertSame([], $ignored, sprintf(
            "Токен тинта не следует за акцентом (%d значений):
%s",
